...
<style>
	...
	#adminDiv+div>table td>div{border: 1px solid black; height: 80%; width: 45%; display: inline-block;
...
<body>
	...
	<div id="container">
		...
		<!-- 
			니즈 : 사원 리스트 뽑기
			-> 조회한 결과를 tr로 반복(foreach)해서 출력
			-> AdminViewServlet에서 관리자 계정이면 
				DB에 가서 사원 정보 추출해오고 admin.jsp로 전달
		-->
		<!-- ${empList} -->
		...
		<div class="menuContent">
			...
			<!-- 
				${contextPath} : blueRice/url 처럼 나타내기 위해서
				action="insertEmp.me"
				action="/insertEmp.me"
				action="/blueRice/insertEmp.me"
			 -->
	...
	<script>
		...
		window.onload = () => {
			...
			// 사원번호 중복확인 -> ajax(jq버전)이용 + submit불가도 추가
			document.getElementById('id').addEventListener('focusout', e => {
				
				//const value = this.value; 화살표함수 말고 function(){}을 사용해야 함
				const value = e.target.value.trim(); // trim() : 공백제거
				...
				if(value != ''){
					...
					targetTd.innerText = ''; // 사원번호를 지우면 사용가능 불가능도 같이 사라지게 만드는 코드
				}
			});
				...
				button.addEventListener('click', function() => {
					if(this.className == 'unselectState'){
						...
						$.ajax({
							...
							success: data => {
								//console.log(data);
								if(data.trim() == 'success'){ // println때문에 줄바꿈 공백 생김
